...
from ._formula_splines import _bspline_basis_1d_from_state, _bspline_basis_1d_predict, _bspline_state_1d
            ...
            for term in self.terms:
                ...
                if isinstance(term, SmoothTerm):
                    ...
                    if len(term.vars) == 1:
                        v1 = term.vars[0]
                        x1 = np.asarray(frame[v1]).reshape(-1)
                        B = _bspline_basis_1d_predict(x1, st)
                        if term.by is None:
                            cols.append(B)
                        else:
                    ...
                    if len(term.vars) == 2:
                        ...
                        x1 = np.asarray(frame[v1]).reshape(-1)
                        x2 = np.asarray(frame[v2]).reshape(-1)
                        B1 = _bspline_basis_1d_predict(x1, st["state1"])
                        B2 = _bspline_basis_1d_predict(x2, st["state2"])
                        B = (B1[:, :, None] * B2[:, None, :]).reshape(x1.shape[0], -1)
                        if term.by is None:
                            cols.append(B)
                        else:
